...
namespace AI.Genetics
{
    public class GeneticAlgorithm : MonoBehaviour
    {
        ...
        public GameObject bestIndividualMarker;
        ...
        IEnumerator GeneticCO(List<Genotype> initialGenotypes)
        {
            ...
            for (int i = 0; i < nGenerations; i++)
            {
                ...
                // Selection
                currentGenotypes.Sort((x, y) => (int)((y.fitness - x.fitness) * 100));
                ...
                Debug.Log("Generation " + i + " - Best Fitness: " + selectedGenotypes[0].fitness + " - Second best Fitness: " + selectedGenotypes[1].fitness);

                SetBestIndividualMarker(selectedGenotypes[0].fitness);
            }
        }

        private void SetBestIndividualMarker(float newY)
        {
            var firstPos = bestIndividualMarker.transform.position;
            if (newY > firstPos.y)
            {
                var newPos = firstPos;
                newPos.y = newY;
                bestIndividualMarker.transform.position = newPos;
            }
        }
    }
}
